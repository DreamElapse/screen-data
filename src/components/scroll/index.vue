...
export default {
  ...
  methods: {
    ...
    horizonMove() {
      if (!this.switch || this.list.length < this.col || this.listCopy.length < this.col) {
        return;
      }
      //上半部分
      let up = this.$refs['up'];
    ...
    verticalMove() {
      if (!this.switch || this.list.length < this.row || this.listCopy.length < this.row) {
        return;
      }
      ...
      let bottom2 = parseInt(up.style.bottom);
      let bottom1 = parseInt(down.style.bottom);

      bottom1 = bottom1 - this.itemHeight;
      bottom2 = bottom2 - this.itemHeight;

      if (bottom1 < -this.height) {
        down.style.visibility = 'hidden';
        down.style.bottom = `${this.height}px`;
        //延时执行
        setTimeout(() => {
          down.style.visibility = 'visible';
        }, 300);
      } else {
        down.style.bottom = bottom1 + 'px';
        down.style.visibility = 'visible';
        if (bottom1 === -this.height) {
          //延时等待transition 执行完成再list的调整位置
          setTimeout(() => {
      ...
      if (bottom2 < -this.height) {
        up.style.visibility = 'hidden';
        up.style.bottom = `${this.height}px`;
        setTimeout(() => {
          up.style.visibility = 'visible';
        }, 300);
      } else {
        up.style.bottom = bottom2 + 'px';
        up.style.visibility = 'visible';
        if (bottom2 === -this.height) {
          setTimeout(() => {
            up.style.bottom = `${this.height}px`;
